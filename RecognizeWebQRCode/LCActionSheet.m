...
#pragma mark - methods
+ (instancetype)sheetWithTitle:(NSString *)title buttonTitles:(NSArray *)buttonTitles specialButtonIndex:(NSInteger)redButtonIndex delegate:(id<LCActionSheetDelegate>)delegate {
    
    return [[self alloc] initWithTitle:title buttonTitles:buttonTitles specialButtonIndex:redButtonIndex delegate:delegate];
}

+ (instancetype)sheetWithTitle:(NSString *)title buttonTitles:(NSArray *)buttonTitles specialButtonIndex:(NSInteger)redButtonIndex clicked:(LCActionSheetBlock)clicked {
    
    return [[self alloc] initWithTitle:title buttonTitles:buttonTitles specialButtonIndex:redButtonIndex clicked:clicked];
}

- (instancetype)initWithTitle:(NSString *)title
                 buttonTitles:(NSArray *)buttonTitles
               specialButtonIndex:(NSInteger)redButtonIndex
    ...
    if (self = [super init]) {
        ...
        self.specialButtonIndex = redButtonIndex;
...
- (void)setupMainView {
    ...
    if (self.buttonTitles.count) {
        ...
        for (int i = 0; i < self.buttonTitles.count; i++) {
            ...
            //如果设置了不能点击的按钮
            if (self.unFunctionalBtn&&[self.buttonTitles[i] isEqualToString:self.unFunctionalBtn]) {
                [btn setTitleColor:[UIColor colorWithRed: 0.784 green: 0.784 blue: 0.784 alpha: 1] forState:UIControlStateNormal];
